...
const determineStatus = ({ hasValidation, isDisabled, issue, group, fileName }) => {
...
readme = readme.replace(/<!--table:(.*?)-->(.*?)<!--table-end-->/gs, (_, group) => {
  ...
  for (const file of files) {
    ...
    const status = determineStatus({ hasValidation, isDisabled: !testable, issue, group, fileName: file });
